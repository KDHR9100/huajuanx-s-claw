...
function readNewEvents(agentId, sinceMs, capChars) {
  ...
  for (const r of rows) {
    ...
    const line = `[${new Date(r.created_at).toTimeString().slice(0, 5)}] ${role === 'user' ? '<真名已移除>' : 'Rana'}: ${text.replace(/\s+/g, ' ').slice(0, 500)}`;
...
function parseEntries(raw, tag, fallbackTs) {
  ...
  for (let ln of raw.split('\n')) {
    ...
    if (!/<真名已移除>|Rana/.test(ln)) continue; // 纪要必须提到他们俩之一
